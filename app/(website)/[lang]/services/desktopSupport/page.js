...
import { getDestokSupportPage, getPostById } from "@/lib/sanity/client";
...
export default async function DesktopSupportPage({ params }) {
  // Fetch data and post
  const data = await getDestokSupportPage(params.lang);
  ...
  return (
    <Suspense fallback={<Loading />}>
      {data?.[0] ? (
        <DesktopSupport data={data[0]} post={post} lang={params.lang} />
      ) : (
